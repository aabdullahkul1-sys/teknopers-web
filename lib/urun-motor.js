...
var MARKA = 'TEKNOPERS';
var FOTO_SIRKET='<img src="/gorseller/depo-stok-takip-tablet.webp" width="1200" height="800" loading="lazy" decoding="async" alt="İşletme sahibi tablet ile stok ve ekip takibi yapıyor — TeknoPers" style="display:block;width:100%;height:auto;border-radius:18px;margin:22px 0 4px;box-shadow:0 16px 40px -16px rgba(11,18,32,.30)">\n';
var FOTO_FATURA='<img src="/gorseller/fatura-fotograf-stok-aktarma.webp" width="1200" height="800" loading="lazy" decoding="async" alt="Faturayı telefonla çekip ürünleri stoğa aktaran çalışan — TeknoPers akıllı fatura okuma" style="display:block;width:100%;height:auto;border-radius:8px;margin:8px 0 22px;box-shadow:0 12px 30px -14px rgba(11,18,32,.28)">\n';
var FOTO_SALON='<img src="/gorseller/salon-randevu-takvimi.webp" width="1200" height="800" loading="lazy" decoding="async" alt="Salon çalışanı tablette randevu takvimini yönetiyor — TeknoPersSalon" style="display:block;width:100%;height:auto;border-radius:18px;margin:22px 0 4px;box-shadow:0 16px 40px -16px rgba(11,18,32,.30)">\n';

function urunFoto(o){
  var t = ((o.ad||'')+' '+(o.keyword||'')+' '+(o.slug||'')).toLocaleLowerCase('tr');
  var img, alt;
  if(o.urunTuru==='salon'){
    if(/diş|dis-|dental/.test(t)){ img='dis-klinik-randevu'; alt='Diş kliniğinde randevu ve hasta kayıtlarını tablet ile yöneten ekip — TeknoPers'; }
    else if(/veteriner|hayvan/.test(t)){ img='veteriner-klinik-randevu'; alt='Veteriner kliniğinde müşteri ve randevu takibi — TeknoPers'; }
    else if(/güzellik|guzellik|estetik|cilt|spa|masaj/.test(t)){ img='guzellik-estetik-randevu'; alt='Güzellik ve estetik merkezinde randevu takibi — TeknoPers'; }
    else if(/kuaför|kuafor|berber|dövme|dovme|tırnak|tirnak/.test(t)){ img='kuafor-salon-yogun'; alt='Yoğun kuaför salonunda randevu ve müşteri yönetimi — TeknoPersSalon'; }
    else if(/diyet|pilates|fizyo|klinik|sağlık|saglik/.test(t)){ img='klinik-diyetisyen-randevu'; alt='Klinik ve stüdyoda randevu takibi — TeknoPers randevu'; }
    else { img='salon-randevu-takvimi'; alt='Salon çalışanı tablette randevu takvimini yönetiyor — TeknoPersSalon'; }
  } else {
    if(/pdks|puantaj|personel|mesai|vardiya|izin/.test(t)){ img='saha-personel-pdks-insaat'; alt='Sahada personel giriş-çıkış ve puantaj takibi — TeknoPers'; }
    else if(/fatura|muhasebe|irsaliye/.test(t)){ img='fatura-fotograf-stok-aktarma'; alt='Faturayı telefonla çekip stoğa aktarma — TeknoPers'; }
    else if(/stok|depo|barkod|zimmet/.test(t)){ img='depo-stok-kontrolu-tablet'; alt='Depoda tablet ile stok kontrolü — TeknoPers'; }
    else { img='depo-stok-takip-tablet'; alt='İşletme sahibi tablet ile stok ve ekip takibi yapıyor — TeknoPers'; }
  }
  return '<img src="/gorseller/'+img+'.webp" width="1200" height="800" loading="lazy" decoding="async" alt="'+alt+'" style="display:block;width:100%;height:auto;border-radius:8px;margin:20px 0 4px;box-shadow:0 16px 40px -16px rgba(11,18,32,.30)">\n';
}
...
var PSTIL = [
  ...
  '.u-faq p{margin:0 16px 14px;color:var(--slate);font-size:14.5px;line-height:1.66}','.af .ic svg{color:#9A6B00}','body.tema-salon .af .ic svg{color:#A23A4E}'
].join('');

function altOzellikGrid(list){
  if(!list||!list.length) return '';
  return '<div class="af-grid">'+list.map(function(o){
    return '<div class="af"><b>'+esc(o.b)+'</b><span>'+esc(o.a)+'</span></div>';
  }).join('')+'</div>';
}
...
function urunSayfaUret(o, sektorler, sorular){
  ...
  '<div><div class="old"><s>'+trSayi(f.yillik)+' ₺/yıl normal fiyat</s></div>'+
  '<div class="big">1.000 ₺<small>/ilk yıl</small></div>'+
  '<div style="font-family:var(--mono);font-size:11.5px;font-weight:700;letter-spacing:.06em;text-transform:uppercase;color:var(--goldD);margin-top:6px">Lansman kampanyası</div></div>'+
  '</div><div class="price-rozet">1 yıl 1.000 ₺ · 15 gün ücretsiz dene</div>'+
  '<div class="price-note">Lansman kampanyası: ilk yıl 1.000 ₺ (yıllık tek ödeme, sunucu maliyetine yakın). Normal fiyat üstte gösterilir. KDV hariç · sınırsız personel · kurulum + eğitim + destek dahil.</div>'+
...
function urunListeUret(urunler){
  ...
  var bolum=Object.keys(grup).map(function(k){
    var kart=grup[k].map(function(u){
      return '<a href="/urun/'+u.slug+'" class="af" style="display:block;text-decoration:none"><b>'+esc(u.ad)+'</b><span>'+esc(kirp((u.hero&&u.hero[0])||'',110))+'</span></a>';
    }).join('');
